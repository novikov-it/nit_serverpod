...
class CustomFileUploader {
  ...
  MediaType? _typeMedia;

  /// Creates a new FileUploader from an [uploadDescription] created by the
  /// server.
  CustomFileUploader(String uploadDescription, MediaType? mime) {
    ...
    if (_uploadDescription.type == _UploadType.binary) {
      try {
        var result = await http.post(
          ...
          headers: {
            'Content-Type': _typeMedia == MediaType.voice
                ? 'audio/x-m4a'
      ...
      var multipartFile =
          http.MultipartFile(_uploadDescription.field!, stream, length,
              filename: _uploadDescription.fileName,
              contentType: _typeMedia == MediaType.video
                  ? parser.MediaType('video', 'mp4')
                  : _typeMedia == MediaType.voice
                      ? parser.MediaType('audio', 'x-m4a')
                      : null);
      ...
      if (_typeMedia == MediaType.video) {
        request.fields['Content-Type'] = 'video/mp4';
      } else if (_typeMedia == MediaType.voice) {
        request.fields['Content-Type'] = 'audio/x-m4a';
      } else {
        request.fields['Content-Type'] = 'otcet-stream';
      }
